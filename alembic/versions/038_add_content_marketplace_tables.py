...
def upgrade() -> None:
    ...
    if not result:
        op.create_table(
            ...
            sa.Column('content_type', postgresql.ENUM('study_guide', 'flashcard_deck', 'summary_notes', 'practice_quiz', 'video_tutorial', 'cheat_sheet', name='contenttype', create_type=False), nullable=False),
            ...
            sa.Column('status', postgresql.ENUM('draft', 'pending_review', 'approved', 'rejected', 'flagged', 'archived', name='contentstatus', create_type=False), nullable=False, server_default='draft'),
            sa.Column('moderation_status', postgresql.ENUM('pending', 'in_review', 'approved', 'rejected', 'needs_revision', name='moderationstatus', create_type=False), nullable=False, server_default='pending'),
            sa.Column('plagiarism_status', postgresql.ENUM('not_checked', 'checking', 'passed', 'failed', 'under_review', name='plagiarismstatus', create_type=False), nullable=False, server_default='not_checked'),
    ...
    if not result:
        op.create_table(
            ...
            sa.Column('moderation_status', postgresql.ENUM('pending', 'in_review', 'approved', 'rejected', 'needs_revision', name='moderationstatus', create_type=False), nullable=False),
            ...
            sa.Column('quality_checks', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    if not result:
        op.create_table(
            ...
            sa.Column('institution_id', sa.Integer(), nullable=False),
            sa.Column('content_id', sa.Integer(), nullable=False),
            sa.Column('plagiarism_status', postgresql.ENUM('not_checked', 'checking', 'passed', 'failed', 'under_review', name='plagiarismstatus', create_type=False), nullable=False),
            sa.Column('similarity_score', sa.Float(), nullable=True),
            sa.Column('sources_found', sa.Integer(), nullable=False, server_default='0'),
            sa.Column('matched_contents', postgresql.JSON(astext_type=sa.Text()), nullable=True),
            sa.Column('external_sources', postgresql.JSON(astext_type=sa.Text()), nullable=True),
            sa.Column('check_details', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    if not result:
        op.create_table(
            ...
            sa.Column('transaction_type', postgresql.ENUM('earn_sale', 'spend_purchase', 'admin_credit', 'admin_debit', 'refund', 'withdrawal', 'bonus', name='transactiontype', create_type=False), nullable=False),
