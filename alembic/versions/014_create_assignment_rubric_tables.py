...
def upgrade() -> None:
    ...
    # Only create rubric tables if assignments table exists
    if 'assignments' not in inspector.get_table_names():
        return
    
    if 'rubric_criteria' not in inspector.get_table_names():
        op.create_table(
            'rubric_criteria',
    ...
    if 'rubric_levels' not in inspector.get_table_names():
        op.create_table(
            'rubric_levels',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    if 'submission_grades' not in inspector.get_table_names() and 'submissions' in inspector.get_table_names():
        op.create_table(
            'submission_grades',
            sa.Column('id', sa.Integer(), nullable=False),
...
def downgrade() -> None:
    ...
    if 'submission_grades' in inspector.get_table_names():
    ...
    if 'rubric_levels' in inspector.get_table_names():
